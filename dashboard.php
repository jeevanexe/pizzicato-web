...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-badge-alert { background:rgba(231,76,60,.15) !important; color:#991b1b !important; }
        ...
        .pill-red   { background:rgba(231,76,60,.12); color:#991b1b; }
        ...
        .especificos-add { display:flex; gap:.5rem; align-items:center; flex-wrap:wrap; margin-top:.5rem; }
        .especificos-add input[type=date], .especificos-add input[type=time], .especificos-add select { border:1px solid var(--light); border-radius:.65rem; padding:.5rem .75rem; font-family:'DM Sans',sans-serif; font-size:.875rem; background:var(--cream); color:var(--dark); outline:none; }
        .especificos-add input[type=date]:focus, .especificos-add input[type=time]:focus, .especificos-add select:focus { border-color:var(--brown); }
        .esp-row { display:flex; gap:.5rem; align-items:center; justify-content:space-between; padding:.5rem .75rem; background:var(--cream); border-radius:.65rem; border:1px solid var(--light); }
        .esp-row-info { font-size:.82rem; color:var(--dark); flex:1; }
        .section-divider { height:1px; background:var(--light); margin:1rem 0; grid-column:1/-1; }
        .section-subtitle { font-size:.78rem; font-weight:600; color:rgba(45,36,36,.5); text-transform:uppercase; letter-spacing:.1em; grid-column:1/-1; margin-top:.5rem; }
        ...
        /* MENSAJE MODAL */
        .mensaje-body { padding:1.5rem; }
        .mensaje-meta { display:grid; grid-template-columns:1fr 1fr; gap:.75rem; margin-bottom:1.25rem; }
        .mensaje-meta-item { background:rgba(245,230,211,.4); border-radius:.65rem; padding:.65rem .9rem; }
        .mensaje-meta-item label { font-size:.7rem; font-weight:500; color:rgba(45,36,36,.5); text-transform:uppercase; letter-spacing:.08em; display:block; margin-bottom:.2rem; }
        .mensaje-meta-item span { font-size:.875rem; color:var(--dark); word-break:break-all; }
        .mensaje-texto { background:var(--cream); border-radius:.75rem; padding:1rem; font-size:.9rem; line-height:1.65; color:var(--dark); white-space:pre-wrap; border:1px solid var(--light); }
...
<div class="dash-layout">
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <a class="nav-item" data-sec="mensajes" onclick="goTo('mensajes',this)">
            ...
            Mensajes <span class="nav-badge" id="badge-mensajes">—</span>
        </a>
    ...
    <main class="content">
        ...
        <div class="section active" id="sec-inicio">
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    <div class="stat-icon"><svg fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></div>
                    <div class="stat-label">Mensajes sin leer</div>
                    <div class="stat-val" id="stat-mensajes">—</div>
                </div>
            </div>
            <div class="two-col">
                <div class="card">
            ...
            <div class="two-col">
                <div class="card">
                    ...
                    <label for="fileInput" class="upload-zone" id="uploadZone">
                        <div class="upload-icon"><svg fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><polyline points="16 16 12 12 8 16"/><line x1="12" y1="12" x2="12" y2="21"/><path d="M20.39 18.39A5 5 0 0 0 18 9h-1.26A8 8 0 1 0 3 16.3"/></svg></div>
                    ...
                    <div style="padding:0 1.25rem 1.25rem;">
                        <div id="uploadPreview" style="display:none;">
                            ...
                            <img id="previewImg" style="width:100%;border-radius:.65rem;border:1px solid var(--light);display:block;" alt="">
        ...
        <div class="section" id="sec-mensajes">
            <div class="sec-head">
                <div><h1>Mensajes</h1><p>Mensajes recibidos desde el formulario de contacto.</p></div>
                <span id="mensajes-nuevos-badge" style="display:none;background:rgba(231,76,60,.1);color:#991b1b;font-size:.8rem;font-weight:500;padding:.4rem 1rem;border-radius:99px;"></span>
            </div>
            <div class="card">
                <div class="card-header">
...
<div class="modal-overlay" id="modalClase">
    <div class="modal">
        <div class="modal-header">
            ...
            <button class="modal-close" onclick="cerrarModal('modalClase')"><svg fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
        </div>
        <div class="form-grid">
            <div class="form-group"><label>Nombre de la clase *</label><input type="text" id="c-nombre" placeholder="Ej. Piano Clásico"></div>
            <div class="form-group"><label>Profesor</label><input type="text" id="c-profesor" placeholder="Prof. Nombre Apellido"></div>
            <div class="form-group"><label>Instrumento</label>
                <select id="c-instrumento"><option value="piano">Piano</option><option value="guitarra">Guitarra</option><option value="canto">Canto</option><option value="ukulele">Ukulele</option><option value="otro">Otro</option></select>
            </div>
            <div class="form-group"><label>Nivel</label>
                <select id="c-nivel"><option value="todos">Todos los niveles</option><option value="principiante">Principiante</option><option value="intermedio">Intermedio</option><option value="avanzado">Avanzado</option></select>
            </div>
            <div class="form-group"><label>Duración (minutos)</label><input type="number" id="c-duracion" value="60" min="15" max="180"></div>
            <div class="form-group"><label>Precio (MXN)</label><input type="number" id="c-precio" value="200" min="0"></div>
            <div class="form-group"><label>Estado</label>
                <select id="c-estado"><option value="activa">Activa</option><option value="borrador">Borrador</option><option value="inactiva">Inactiva</option></select>
            </div>
            <div class="form-group full">
                <label>Imagen</label>
                <div class="img-input-wrap">
                    <input type="text" id="c-imagen" placeholder="https://..." oninput="syncPreview('c-imagen','c-img-prev-img','c-img-prev')">
                    <button type="button" class="btn-biblioteca" onclick="abrirPicker('c-imagen','c-img-prev-img','c-img-prev')">
                        <svg fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                        Biblioteca
                    </button>
                </div>
                <div class="img-preview-wrap" id="c-img-prev"><img id="c-img-prev-img" src="" alt=""><button type="button" class="img-preview-remove" onclick="quitarPreview('c-imagen','c-img-prev-img','c-img-prev')">✕</button></div>
            </div>
            <div class="form-group full"><label>Descripción</label><textarea id="c-descripcion" placeholder="Describe la clase brevemente..."></textarea></div>
            <div class="section-subtitle">Horarios recurrentes (cada semana)</div>
            <div class="form-group full">
                <div class="horarios-list" id="horariosList"></div>
                <button type="button" class="btn btn-secondary btn-sm" style="align-self:flex-start;margin-top:.25rem" onclick="agregarHorario()">
                    ...
                    Agregar horario recurrente
                </button>
            </div>
            <div class="section-divider"></div>
            <div class="section-subtitle">Fechas específicas (extra o bloqueo)</div>
            <div class="form-group full">
                <div class="horarios-list" id="especificosListVer"></div>
                <div class="especificos-add">
                    <input type="date" id="esp-fecha">
                    <input type="time" id="esp-hora" value="09:00">
                    <select id="esp-tipo"><option value="extra">Extra (día especial)</option><option value="bloqueado">Bloquear (cancelar ese día)</option></select>
                    <button type="button" class="btn btn-secondary btn-sm" onclick="agregarEspecifico()">+ Agregar</button>
                </div>
                <p style="font-size:.75rem;color:rgba(45,36,36,.45);margin-top:.4rem;"><strong>Extra:</strong> agrega disponibilidad puntual. <strong>Bloquear:</strong> cancela ese día aunque sea recurrente.</p>
            </div>
        </div>
...
<div class="modal-overlay" id="modalEvento">
    <div class="modal">
        <div class="modal-header">
            ...
            <button class="modal-close" onclick="cerrarModal('modalEvento')"><svg fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
        </div>
        <div class="form-grid">
            <div class="form-group"><label>Nombre del evento *</label><input type="text" id="e-nombre" placeholder="Ej. Concierto de primavera"></div>
            <div class="form-group"><label>Fecha *</label><input type="date" id="e-fecha"></div>
            <div class="form-group"><label>Hora</label><input type="time" id="e-hora" value="18:00"></div>
            <div class="form-group"><label>Lugar</label><input type="text" id="e-lugar" placeholder="Ej. Sala principal"></div>
            <div class="form-group"><label>Precio de entrada</label><input type="text" id="e-precio" placeholder="Gratis / $100"></div>
            <div class="form-group"><label>Estado</label>
                <select id="e-estado"><option value="proximo">Próximo</option><option value="pasado">Pasado</option><option value="cancelado">Cancelado</option></select>
            </div>
            <div class="form-group full"><label>Descripción</label><textarea id="e-descripcion" placeholder="Describe el evento..."></textarea></div>
            <div class="form-group full">
                <label>Imagen</label>
                <div class="img-input-wrap">
                    <input type="text" id="e-imagen" placeholder="https://..." oninput="syncPreview('e-imagen','e-img-prev-img','e-img-prev')">
                    <button type="button" class="btn-biblioteca" onclick="abrirPicker('e-imagen','e-img-prev-img','e-img-prev')">
                        <svg fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                        Biblioteca
                    </button>
                </div>
                <div class="img-preview-wrap" id="e-img-prev"><img id="e-img-prev-img" src="" alt=""><button type="button" class="img-preview-remove" onclick="quitarPreview('e-imagen','e-img-prev-img','e-img-prev')">✕</button></div>
            </div>
        </div>
        <div class="form-footer">
...
<div class="modal-overlay" id="modalMensaje">
    <div class="modal" style="max-width:36rem;">
        <div class="modal-header">
            <h3 id="modalMensajeTitulo">Mensaje de contacto</h3>
            <button class="modal-close" onclick="cerrarModal('modalMensaje')"><svg fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
        </div>
        <div class="mensaje-body">
            <div class="mensaje-meta">
                <div class="mensaje-meta-item"><label>Nombre</label><span id="mNombre">—</span></div>
                <div class="mensaje-meta-item"><label>Correo</label><span id="mCorreo">—</span></div>
                <div class="mensaje-meta-item"><label>Teléfono</label><span id="mTelefono">—</span></div>
                <div class="mensaje-meta-item"><label>Fecha</label><span id="mFecha">—</span></div>
            </div>
            <p style="font-size:.78rem;font-weight:500;color:rgba(45,36,36,.5);text-transform:uppercase;letter-spacing:.08em;margin-bottom:.5rem;">Mensaje</p>
            <div class="mensaje-texto" id="mTexto"></div>
            <div style="margin-top:1.25rem;display:flex;gap:.75rem;justify-content:flex-end;">
                <a id="mResponder" href="#" class="btn btn-primary" target="_blank">
                ...
                <button class="btn btn-secondary" onclick="cerrarModal('modalMensaje')">Cerrar</button>
            </div>
        </div>
    </div>
</div>
...
let editandoClaseId = null, editandoEventoId = null;
let tagSeleccionado = 'clase', archivoPendiente = null;
let _especificosActuales = [], _especificosNuevos = [];
...
async function cargarTodo() {
    try {
        const [rClases, rEventos, rImagenes, rReservas, rMensajes] = await Promise.all([
            fetch(API.clases), fetch(API.eventos), fetch(API.imagenes),
            fetch(API.reservas), fetch(API.mensajes),
        ]);
        _clases   = await rClases.json();
        _eventos  = await rEventos.json();
        _imagenes = await rImagenes.json();
        _reservas = await rReservas.json();
        _mensajes = await rMensajes.json();
    } catch(err) { toast('Error al conectar con el servidor.', 'error'); }
    renderTodo();
}

function renderTodo() {
    renderBadges(); renderClases(); renderEventos();
    renderImagenes(); renderReservas(); renderInicioClases(); renderInicioEventos(); renderMensajes();
}
...
function renderBadges() {
    ...
    const noLeidos = Array.isArray(_mensajes) ? _mensajes.filter(m => !parseInt(m.leido)).length : 0;
    document.getElementById('stat-mensajes').textContent  = noLeidos || '0';
    const badgeMensajes = document.getElementById('badge-mensajes');
    badgeMensajes.textContent = Array.isArray(_mensajes) ? _mensajes.length : '!';
    noLeidos > 0 ? badgeMensajes.classList.add('nav-badge-alert') : badgeMensajes.classList.remove('nav-badge-alert');
}
...
function renderClases() {
    ...
    if (!t) return;
    if (!Array.isArray(_clases) || !_clases.length) { t.innerHTML=`<tr><td class="tbl-empty" colspan="7">No hay clases registradas.</td></tr>`; return; }
    t.innerHTML = `<thead><tr><th>Clase</th><th>Instrumento</th><th>Nivel</th><th>Duración</th><th>Precio</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>` +
        _clases.map(c => {
            const img = c.imagen_url ? `<img src="${c.imagen_url}" class="cell-thumb" alt="" onerror="this.style.display='none'">` : `<div class="cell-thumb"><svg fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg></div>`;
            return `<tr><td><div class="cell-img">${img}<div><div class="cell-name">${c.nombre}</div><div class="cell-sub">${c.profesor??''}</div></div></div></td>
                ...
                <td><div class="row-actions"><button class="btn btn-secondary btn-sm" onclick="editarClase(${c.id})">Editar</button><button class="btn btn-danger btn-sm" onclick="eliminarClase(${c.id})">Eliminar</button></div></td>
            </tr>`;
        }).join('') + `</tbody>`;
}
...
function renderInicioClases() {
    ...
    if (!_clases.length) { t.innerHTML=`<tr><td class="tbl-empty">Sin clases.</td></tr>`; return; }
    t.innerHTML = `<thead><tr><th>Clase</th><th>Estado</th></tr></thead><tbody>` +
        _clases.slice(0,5).map(c => `<tr><td><div class="cell-name">${c.nombre}</div><div class="cell-sub">${c.profesor??''}</div></td><td>${pillClase(c.estado)}</td></tr>`).join('') + `</tbody>`;
}
...
function agregarHorario(dia='', hora='') {
    ...
    row.innerHTML = `<select>${dias.map(d=>`<option value="${d}" ${d===dia?'selected':''}>${cap(d)}</option>`).join('')}</select><input type="time" value="${hora}"><button type="button" class="btn-del-horario" onclick="this.parentElement.remove()">×</button>`;
    list.appendChild(row);
}

function renderEspecificos() {
    const list = document.getElementById('especificosListVer');
    if (!list) return;
    const todos = [..._especificosActuales.map((e,i)=>({...e,guardado:true,idx:i})), ..._especificosNuevos.map((e,i)=>({...e,guardado:false,idx:i}))];
    if (!todos.length) { list.innerHTML = '<p style="font-size:.8rem;color:rgba(45,36,36,.4);padding:.25rem 0;">Sin fechas específicas aún.</p>'; return; }
    list.innerHTML = todos.map(e => {
        ...
        const pill  = e.tipo==='bloqueado' ? '<span class="pill pill-red" style="font-size:.65rem;">Bloqueado</span>' : '<span class="pill pill-green" style="font-size:.65rem;">Extra</span>';
        const horaStr = e.hora ? e.hora.substring(0,5) : '';
        const btnDel = e.guardado ? `<button type="button" class="btn-del-horario" onclick="borrarEspecificoGuardado(${e.id})">×</button>` : `<button type="button" class="btn-del-horario" onclick="borrarEspecificoNuevo(${e.idx})">×</button>`;
        return `<div class="esp-row"><span class="esp-row-info">${cap(label)}${horaStr?' — '+horaStr:''}</span>${pill}${btnDel}</div>`;
    }).join('');
}

function agregarEspecifico() {
    const fecha = document.getElementById('esp-fecha').value;
    const hora  = document.getElementById('esp-hora').value;
    const tipo  = document.getElementById('esp-tipo').value;
    if (!fecha) { toast('Selecciona una fecha.', 'error'); return; }
    if ([..._especificosActuales,..._especificosNuevos].find(e=>e.fecha===fecha&&e.hora===hora&&e.tipo===tipo)) { toast('Ya existe esa fecha/hora.', 'error'); return; }
    _especificosNuevos.push({ fecha, hora, tipo });
    renderEspecificos();
    document.getElementById('esp-fecha').value = '';
}

function borrarEspecificoNuevo(idx) { _especificosNuevos.splice(idx,1); renderEspecificos(); }

async function borrarEspecificoGuardado(id) {
    try {
        const res = await fetch(`${API.horariosEspecificos}?id=${id}`, { method:'DELETE' });
        const data = await res.json();
        if (!res.ok || data.error) throw new Error(data.error);
        _especificosActuales = _especificosActuales.filter(e => e.id !== id);
        if (editandoClaseId) { const c = _clases.find(x=>x.id===editandoClaseId); if (c) c.horarios_especificos = _especificosActuales; }
        renderEspecificos();
...
function abrirModalClase(id) {
    ...
    _especificosActuales = Array.isArray(c?.horarios_especificos) ? [...c.horarios_especificos] : [];
    ...
    (Array.isArray(c?.horarios) ? c.horarios : []).forEach(h => agregarHorario(h.dia_semana, h.hora?.substring(0,5)));
    renderEspecificos();
...
async function guardarClase() {
    ...
    const horarios = [];
    document.getElementById('horariosList').querySelectorAll('.horario-row').forEach(fila => {
    ...
    const payload = { nombre, profesor:document.getElementById('c-profesor').value.trim(), instrumento:document.getElementById('c-instrumento').value, nivel:document.getElementById('c-nivel').value, duracion:parseInt(document.getElementById('c-duracion').value)||60, precio:parseFloat(document.getElementById('c-precio').value)||0, descripcion:document.getElementById('c-descripcion').value.trim(), imagen_url:document.getElementById('c-imagen').value.trim(), estado:document.getElementById('c-estado').value, horarios };
    ...
    try {
        ...
        const claseId = editandoClaseId ?? data.id;
        for (const e of _especificosNuevos) {
            await fetch(API.horariosEspecificos, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ clase_id:claseId, fecha:e.fecha, hora:e.hora, tipo:e.tipo }) });
        }
        _especificosNuevos = [];
        toast(editandoClaseId ? 'Clase actualizada.' : 'Clase creada.', 'success');
        cerrarModal('modalClase');
        _clases = await (await fetch(API.clases)).json();
        renderClases(); renderInicioClases(); renderBadges();
    } catch(err) {
        toast('Error: ' + err.message, 'error');
        setLoading('btnGuardarClase', false);
        document.getElementById('btnGuardarClase').innerHTML = `<svg fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24" style="width:13px;height:13px;"><polyline points="20 6 9 17 4 12"/></svg> Guardar clase`;
    }
}
...
function renderEventos() {
    ...
    if (!t) return;
    if (!Array.isArray(_eventos) || !_eventos.length) { t.innerHTML=`<tr><td class="tbl-empty" colspan="7">No hay eventos registrados.</td></tr>`; return; }
    t.innerHTML = `<thead><tr><th>Evento</th><th>Fecha</th><th>Hora</th><th>Lugar</th><th>Entrada</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>` +
        _eventos.map(e => `<tr>
            ...
            <td><div class="row-actions"><button class="btn btn-secondary btn-sm" onclick="editarEvento(${e.id})">Editar</button><button class="btn btn-danger btn-sm" onclick="eliminarEvento(${e.id})">Eliminar</button></div></td>
        </tr>`).join('') + `</tbody>`;
}
...
function renderInicioEventos() {
    ...
    if (!proximos.length) { t.innerHTML=`<tr><td class="tbl-empty">Sin próximos eventos.</td></tr>`; return; }
    t.innerHTML = `<thead><tr><th>Evento</th><th>Fecha</th></tr></thead><tbody>` +
        proximos.slice(0,5).map(e => `<tr><td><div class="cell-name">${e.nombre}</div><div class="cell-sub">${e.lugar??''}</div></td><td class="tbl-muted" style="white-space:nowrap">${formatFecha(e.fecha)}</td></tr>`).join('') + `</tbody>`;
}
...
async function guardarEvento() {
    ...
    const payload = { nombre, fecha, hora:document.getElementById('e-hora').value, lugar:document.getElementById('e-lugar').value.trim(), precio_entrada:document.getElementById('e-precio').value.trim(), descripcion:document.getElementById('e-descripcion').value.trim(), imagen_url:document.getElementById('e-imagen').value.trim(), estado:document.getElementById('e-estado').value };
    setLoading('btnGuardarEvento', true);
    try {
        let res;
        if (editandoEventoId) { res = await fetch(`${API.eventos}?id=${editandoEventoId}`, { method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload) }); }
        else { res = await fetch(API.eventos, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload) }); }
        const data = await res.json();
        if (!res.ok || data.error) throw new Error(data.error || `Error ${res.status}`);
        toast(editandoEventoId ? 'Evento actualizado.' : 'Evento creado.', 'success');
        cerrarModal('modalEvento');
        _eventos = await (await fetch(API.eventos)).json();
        renderEventos(); renderInicioEventos(); renderBadges();
    } catch(err) {
        toast('Error: ' + err.message, 'error');
        setLoading('btnGuardarEvento', false);
        document.getElementById('btnGuardarEvento').innerHTML = `<svg fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24" style="width:13px;height:13px;"><polyline points="20 6 9 17 4 12"/></svg> Guardar evento`;
    }
}
...
function renderImagenes() {
    ...
    if (!grid) return;
    if (!Array.isArray(_imagenes) || !_imagenes.length) {
        count.textContent = '0 imágenes';
        grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1"><svg fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg><p>Aún no hay imágenes subidas.</p></div>`;
        return;
    }
    count.textContent = `${_imagenes.length} imagen${_imagenes.length!==1?'es':''}`;
    grid.innerHTML = _imagenes.map(img => `<div class="img-item"><img src="${img.url}" alt="${img.nombre}" onerror="this.src=''"><div class="img-item-overlay"><button class="del-img" onclick="eliminarImagen(${img.id})">Eliminar</button></div><div class="img-label">${img.tag??''}</div></div>`).join('');
}

function selectTag(el, ev) { ev&&ev.preventDefault(); document.querySelectorAll('.upload-tag').forEach(t=>t.classList.remove('selected')); el.classList.add('selected'); tagSeleccionado=el.dataset.tag; }
...
function handleFiles(files) {
    ...
    reader.onload = e => { document.getElementById('previewImg').src=e.target.result; document.getElementById('uploadPreview').style.display='block'; };
...
async function confirmarSubida() {
    ...
    try {
        ...
        cancelarSubida();
        _imagenes = await (await fetch(API.imagenes)).json();
        renderImagenes(); renderBadges();
    } catch(err) {
...
function renderReservas() {
    ...
    badge.textContent = pendientes > 0 ? `${pendientes} pendiente${pendientes>1?'s':''}` : '';
    badge.style.display = pendientes > 0 ? 'inline-block' : 'none';
    const filtradas = _filtroReserva==='todos' ? _reservas : _reservas.filter(r=>r.estado===_filtroReserva);
    document.getElementById('reservas-count').textContent = `${filtradas.length} reserva${filtradas.length!==1?'s':''}`;
    if (!filtradas.length) { t.innerHTML=`<tr><td class="tbl-empty" colspan="7">No hay reservas${_filtroReserva!=='todos'?' con este estado':''}.</td></tr>`; return; }
    t.innerHTML = `<thead><tr><th>Alumno</th><th>Correo</th><th>Clase</th><th>Nivel</th><th>Instrumento</th><th>Fecha</th><th>Hora</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>` +
    filtradas.map(r => {
            const estado = r.estado ?? 'pendiente';
            let acciones = '';
            if (estado==='pendiente') acciones=`<button class="btn btn-aceptar btn-sm" onclick="cambiarEstadoReserva(${r.id},'confirmada')">✓ Aceptar</button><button class="btn btn-rechazar btn-sm" onclick="cambiarEstadoReserva(${r.id},'cancelada')">✕ Rechazar</button>`;
            else if (estado==='confirmada') acciones=`<button class="btn btn-rechazar btn-sm" onclick="cambiarEstadoReserva(${r.id},'cancelada')">✕ Cancelar</button>`;
            else acciones=`<button class="btn btn-aceptar btn-sm" onclick="cambiarEstadoReserva(${r.id},'confirmada')">↩ Reactivar</button>`;
            const nivelPill = r.nivel_musical
                    ? `<span class="pill pill-gray" style="font-size:.65rem;">${r.nivel_musical}</span>`
                    : '—';
            const instrPill = r.tiene_instrumento === 'Sí, tengo mi propio instrumento'
                    ? `<span class="pill pill-green" style="font-size:.65rem;">Sí ✓</span>`
                    : r.tiene_instrumento === 'No, necesito uno del estudio'
                    ? `<span class="pill pill-amber" style="font-size:.65rem;">No</span>`
                    : '—';
            return `<tr><td><div class="cell-name">${r.alumno_nombre??''} ${r.alumno_apellido??''}</div><div class="cell-sub">${r.alumno_tel??''}</div></td><td class="tbl-muted">${r.alumno_correo??'—'}</td><td class="tbl-muted">${r.clase_nombre??'—'}<div class="cell-sub">${r.dia_semana?cap(r.dia_semana):''}</div></td><td class="tbl-muted">${nivelPill}</td><td class="tbl-muted">${instrPill}</td><td class="tbl-muted">${formatFecha(r.fecha)}</td><td class="tbl-muted">${r.hora?r.hora.substring(0,5):'—'}</td><td>${pillReserva(estado)}</td><td><div class="row-actions">${acciones}</div></td></tr>`;
        }).join('') + `</tbody>`;
}

async function cambiarEstadoReserva(id, nuevoEstado) {
    const etiquetas = { confirmada:'aceptar', cancelada:'rechazar/cancelar' };
    if (!confirm(`¿Deseas ${etiquetas[nuevoEstado]??nuevoEstado} esta reserva?`)) return;
    try {
        const res = await fetch(`${API.reservas}?id=${id}`, { method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ estado:nuevoEstado }) });
        const data = await res.json();
        if (!res.ok || data.error) throw new Error(data.error || `Error ${res.status}`);
        const idx = _reservas.findIndex(r=>r.id===id);
        if (idx!==-1) _reservas[idx].estado = nuevoEstado;
        const msgs = { confirmada:'Reserva aceptada ✓', cancelada:'Reserva rechazada.' };
        toast(msgs[nuevoEstado]??'Estado actualizado.', nuevoEstado==='confirmada'?'success':'error');
        renderReservas(); renderBadges();
    } catch(err) { toast('Error: ' + err.message, 'error'); }
}

// ── MENSAJES ──
function renderMensajes() {
    const t = document.getElementById('tbl-mensajes');
    if (!t || !Array.isArray(_mensajes)) return;
    const noLeidos = _mensajes.filter(m => !parseInt(m.leido)).length;
    const badge = document.getElementById('mensajes-nuevos-badge');
    badge.textContent  = noLeidos > 0 ? `${noLeidos} sin leer` : '';
    badge.style.display = noLeidos > 0 ? 'inline-block' : 'none';
    document.getElementById('mensajes-count').textContent = `${_mensajes.length} mensaje${_mensajes.length!==1?'s':''}`;
    if (!_mensajes.length) { t.innerHTML=`<tr><td class="tbl-empty" colspan="5">No hay mensajes aún.</td></tr>`; return; }
    t.innerHTML = `<thead><tr><th>Nombre</th><th>Correo</th><th>Asunto</th><th>Fecha</th><th>Acciones</th></tr></thead><tbody>` +
        _mensajes.map(m => {
            const leido    = parseInt(m.leido);
            const fechaRaw = m.created_at ? m.created_at.split(' ')[0] : '';
            const pill     = leido ? '<span class="pill pill-gray" style="font-size:.65rem;">Leído</span>' : '<span class="pill pill-blue" style="font-size:.65rem;">Nuevo</span>';
            return `<tr class="${leido?'':'no-leido'}">
                <td><div class="cell-name">${m.nombre}</div><div class="cell-sub">${m.telefono||''}</div></td>
                <td class="tbl-muted">${m.correo}</td>
                <td class="tbl-muted">${m.asunto||'—'} ${pill}</td>
                <td class="tbl-muted" style="white-space:nowrap">${formatFecha(fechaRaw)}</td>
                <td><div class="row-actions">
                    <button class="btn btn-secondary btn-sm" onclick="verMensaje(${m.id})">Ver</button>
                    ${!leido ? `<button class="btn btn-secondary btn-sm" onclick="marcarLeido(${m.id})">Marcar leído</button>` : ''}
                </div></td>
            </tr>`;
        }).join('') + `</tbody>`;
}
...
function verMensaje(id) {
    ...
    if (!m) return;
    const fechaRaw = m.created_at ? m.created_at.split(' ')[0] : '';
    document.getElementById('modalMensajeTitulo').textContent = m.asunto || 'Mensaje de contacto';
    document.getElementById('mNombre').textContent   = m.nombre;
    document.getElementById('mCorreo').textContent   = m.correo;
    document.getElementById('mTelefono').textContent = m.telefono || '—';
    document.getElementById('mFecha').textContent    = formatFecha(fechaRaw);
    document.getElementById('mTexto').textContent    = m.mensaje;
    document.getElementById('mResponder').href       = `mailto:${m.correo}?subject=Re: ${encodeURIComponent(m.asunto||'Tu mensaje')}`;
    document.getElementById('modalMensaje').classList.add('open');
    if (!parseInt(m.leido)) marcarLeido(id);
}

async function marcarLeido(id) {
    try {
        await fetch(`${API.mensajes}?id=${id}`, { method:'PUT' });
        const idx = _mensajes.findIndex(m => m.id === id);
        if (idx !== -1) _mensajes[idx].leido = 1;
        renderMensajes(); renderBadges();
    } catch(err) { toast('Error: ' + err.message, 'error'); }
}

// ── MODALES ──
function cerrarModal(id) { document.getElementById(id).classList.remove('open'); }
document.querySelectorAll('.modal-overlay').forEach(m => { m.addEventListener('click', e => { if (e.target===m) cerrarModal(m.id); }); });
...
// ── BIBLIOTECA PICKER ──
let _pickerInput=null, _pickerImg=null, _pickerWrap=null;

function abrirPicker(inputId, imgId, wrapId) {
    _pickerInput=inputId; _pickerImg=imgId; _pickerWrap=wrapId;
    const content = document.getElementById('pickerContent');
    if (!Array.isArray(_imagenes)||!_imagenes.length) {
        content.innerHTML=`<div style="text-align:center;padding:3rem 1rem;"><p style="font-size:.9rem;color:rgba(45,36,36,.4);">No hay imágenes en la biblioteca.</p></div>`;
    } else {
        content.innerHTML=`<div class="bib-grid">${_imagenes.map(img=>`<div class="bib-item" onclick="seleccionarImagen('${img.url.replace(/'/g,"\\'")}')"><img src="${img.url}" alt="${img.nombre}" loading="lazy"><div class="bib-item-label">${img.tag??''}</div><div class="bib-item-check"><svg fill="none" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24" style="width:10px;height:10px;"><polyline points="20 6 9 17 4 12"/></svg></div></div>`).join('')}</div>`;
    }
...
function seleccionarImagen(url) {
    ...
    document.getElementById(_pickerImg).src = url;
    document.getElementById(_pickerWrap).style.display = 'block';
    cerrarModal('modalPicker');
}

function syncPreview(inputId, imgId, wrapId) {
    const url = document.getElementById(inputId).value.trim();
    document.getElementById(wrapId).style.display = url ? 'block' : 'none';
...
<div class="modal-overlay" id="modalPicker" style="z-index:600;">
    <div class="modal" style="max-width:48rem;">
        <div class="modal-header">
            <div><h3>Seleccionar imagen</h3><p style="font-size:.8rem;color:rgba(45,36,36,.5);margin-top:.15rem;">Haz clic en una imagen para usarla.</p></div>
            <button class="modal-close" onclick="cerrarModal('modalPicker')"><svg fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
        </div>
